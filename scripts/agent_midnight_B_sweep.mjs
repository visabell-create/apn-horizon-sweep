/**
 * Agent midnight-B exclusive sweep: 8448-011, 022, 027
 * Vacant land = VALID. +1 walk. Hard stop by midnight local (or HARD_STREAK).
 */
...
const PAGES = [11, 22, 27];
const HARD_STREAK = 20;
const CONCURRENCY = 8;
const DEADLINE = new Date("2026-07-15T00:00:00-07:00").getTime();
const PREFIX = "RUN-B-MIDNIGHT";
    ...
    const req = https.request(
      {
        ...
        headers: {
          ...
          "User-Agent": "apn-horizon-sweep-midnight-B/1.0",
        },
        timeout: 30000,
      },
      (res) => {
...
async function walkPage(book, page) {
  ...
  console.log(`[${pageKey}] start @ ${new Date().toISOString()}`);
  const valids = [];
  const vacants = [];
  const trace = [];
  ...
  while (parcel <= 999 && consecutiveInvalid < HARD_STREAK) {
    if (pastDeadline()) {
      stoppedReason = "MIDNIGHT_DEADLINE";
      break;
    }
  ...
  const result = {
    agent: "agent-midnight-B",
    page: pageKey,
    algorithm: "page-walk-20-invalid-streak-midnight-cap",
  ...
  const dest = path.join(ROOT, "runs", `${PREFIX}-${pageKey}`);
...
async function main() {
  ...
  console.log(`midnight-B start ${started}; deadline ${new Date(DEADLINE).toISOString()}`);
  if (pastDeadline()) {
    console.log("Already past midnight — abort");
    process.exit(0);
  }

  // Sequential pages so we finish/archive what we can before midnight
  const results = [];
  for (const p of PAGES) {
    if (pastDeadline()) {
      console.log(`Skipping page ${p} — midnight`);
      break;
    }
    results.push(await walkPage(8448, p));
  }
  ...
  const aggregate = {
    ...
    claimedPages: ["8448-011", "8448-022", "8448-027"],
    startedAt: started,
    finishedAt,
  ...
  const aggDir = path.join(ROOT, "runs", `${PREFIX}-aggregate`);
  fs.mkdirSync(aggDir, { recursive: true });
  fs.writeFileSync(path.join(aggDir, "summary.json"), JSON.stringify(aggregate, null, 2));
  fs.writeFileSync(path.join(aggDir, "full.json"), JSON.stringify(aggregate, null, 2));
  fs.writeFileSync(path.join(aggDir, "valids.json"), JSON.stringify(results.flatMap((r) => r.valids), null, 2));
  fs.writeFileSync(
    path.join(aggDir, "summary.md"),
    `# ${PREFIX} aggregate (8448-011,022,027)

- Total valids: ${aggregate.totalValids}
- Total vacant land: ${aggregate.totalVacants}
- Started: ${aggregate.startedAt}
...
`
  ...
  const status = results.every((r) => r.stoppedReason !== "MIDNIGHT_DEADLINE" && r.stoppedReason !== "UNKNOWN")
    ? "completed"
    : pastDeadline()
      ? "stopped_at_midnight"
      : "completed";
  ...
  fs.writeFileSync(
    path.join(ROOT, "state", "claims", "agent-midnight-B.json"),
    JSON.stringify(
      {
        agent: "agent-midnight-B",
        note: "Second worker until midnight; exclusive +1 walks; vacant=valid; skip serial 006",
        pages: ["8448-011", "8448-022", "8448-027"],
        exclusive: true,
        claimedAt: "2026-07-14T23:57:40-07:00",
        startedAt: started,
        finishedAt,
        deadline: "2026-07-15T00:00:00-07:00",
        status,
        reason: "Dense horizon hits=2 unfinished; near 006-020 / next gaps; not donePages; not serial 006",
        runArchivePrefix: "runs/RUN-B-MIDNIGHT-",
        serialAvoid: "8448-006",
        validCount: aggregate.totalValids,
        vacantCount: aggregate.totalVacants,
        byPage: aggregate.byPage,
      },
      null,
      2
    )
  ...
  fs.writeFileSync(
    path.join(ROOT, "state", "status", "agent-midnight-B.md"),
    `# agent-midnight-B status

- **Agent:** agent-midnight-B (second worker until midnight)
- **Exclusive pages:** 8448-011, 8448-022, 8448-027
- **Claim:** ${status}
- **Claimed:** 2026-07-14 ~11:57 PM local
- **Started:** ${started}
- **Finished:** ${finishedAt}
- **Deadline:** 12:00 AM local — hard stop honored
- **Skip:** 8448-006 (serial walker)
- **Valids:** ${aggregate.totalValids} (vacant land: ${aggregate.totalVacants})
- **By page:**
${aggregate.byPage.map((p) => `  - ${p.page}: ${p.validCount} valids / ${p.vacantCount} vacant — ${p.stoppedReason} @ ${p.lastAinChecked}`).join("\n")}
- **Archives:**
${results.map((r) => `  - \`runs/${PREFIX}-${r.page}/\``).join("\n")}
  - \`runs/${PREFIX}-aggregate/\`
`
  );
